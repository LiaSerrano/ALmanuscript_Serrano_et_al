...
def run_training(args: TrainArgs,
    ...
    train_df = pd.DataFrame({"SMILES":[s[0] for s in train_smiles],
                            "inhibition":[t[0] for t in train_targets]})
    
    validation_smiles, validation_targets = val_data.smiles(), val_data.targets()
    val_df = pd.DataFrame({"SMILES":[s[0] for s in validation_smiles],
                            "inhibition":[t[0] for t in validation_targets]})

    test_smiles, test_targets = test_data.smiles(), test_data.targets()
    test_df = pd.DataFrame({"SMILES":[s[0] for s in test_smiles],
                            "inhibition":[t[0] for t in test_targets]})
    
    train_df.to_csv(os.path.join(args.save_dir, 'train_data.csv'),
                   index = False)
    val_df.to_csv(os.path.join(args.save_dir, 'val_data.csv'),
                   index = False)
    test_df.to_csv(os.path.join(args.save_dir, 'test_data.csv'),
                   index = False)
